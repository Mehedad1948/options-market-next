...
const TAG_CONFIG: Record<string, { label: string; icon: any }> = {
  ...
  leverage_tag: { label: 'اهرم', icon: Gauge }, // Handle both keys if necessary
...
export function DetailsModal({ isOpen, onClose, data }: { isOpen: boolean; onClose: () => void; data: any }) {
  ...
  // Helper to render a grid of candidates (Call or Put)
  const renderCandidateSection = (title: string, items: any[], type: 'CALL' | 'PUT') => {
    if (!items || items.length === 0) return null;

    const isCall = type === 'CALL';
    const HeaderIcon = isCall ? ArrowUpCircle : ArrowDownCircle;
    const headerColor = isCall ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400';
    const itemBorderHover = isCall ? 'hover:border-emerald-300 dark:hover:border-emerald-700' : 'hover:border-rose-300 dark:hover:border-rose-700';

    return (
      <div className="mt-6">
        <h4 className={`font-bold mb-3 flex items-center gap-2 text-lg ${headerColor}`}>
          <HeaderIcon className="w-5 h-5" />
          {title}
        </h4>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
          {items.map((item: any, index: number) => {
            // Handle structure: item.symbol (string) AND item.data.price (number)
            const symbol = item.symbol || '---';
            const price = item.data?.price || item.price; // Fallback
  ...
  const renderAdviceCard = (type: 'CALL' | 'PUT', advice: any) => {
    ...
    return (
      <div className={`w-full border rounded-2xl overflow-hidden flex flex-col ${borderColor} dark:bg-gray-900/50 shadow-sm mb-6`}>
        {/* Card Header */}
        <div className={`p-4 ${bgHeader} border-b ${borderColor} flex justify-between items-center`}>
          ...
          <span className={`px-4 py-1.5 rounded-full text-sm font-bold shadow-sm ${isBuy ? 'bg-green-600 text-white' : 'bg-gray-600 text-white'}`}>
            {advice.decision}
          </span>
        </div>
        ...
        <div className="p-5 space-y-6">
          ...
          <div className="flex justify-between items-end border-b border-gray-100 dark:border-gray-800 pb-5">
            ...
            <div className="text-left">
              ...
              <p className="text-2xl font-bold text-gray-900 dark:text-white">
                {formatPrice(advice.entry_price)} <span className="text-sm font-normal text-gray-400">ریال</span>
              </p>
            </div>
          </div>

          {/* 2. Detailed Tags Grid (Larger Text) */}
          {advice.tags && (
              ...
              {Object.entries(advice.tags).map(([key, value]: any) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="text-sm font-black text-right truncate leading-tight" title={value}>
                      {value}
                    </span>
                  </div>
                );
          ...
          {/* 4. AI Reasoning (Specific) */}
          {advice.reasoning && (
            <div>
              <p className="text-sm font-bold text-gray-400 mb-2">تحلیل تکنیکال:</p>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200" style={{ zIndex: 9999 }}>
      ...
      <div className="bg-white dark:bg-gray-950 rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col border border-gray-200 dark:border-gray-800">
        ...
        {/* Scrollable Content */}
        <div className="p-6 md:p-8 overflow-y-auto space-y-8 text-right bg-gray-50/50 dark:bg-black/20" dir="rtl">
          ...
          {/* Section 2: Strategy Cards (Full Width Stack) */}
          ...
          {(data.candidates?.calls?.length > 0 || data.candidates?.puts?.length > 0) && (
            <div className="border-t border-gray-200 dark:border-gray-800 pt-8">
              <div className="flex items-center gap-2 mb-6 opacity-70">
                <Search className="w-6 h-6" />
                <h3 className="text-base font-bold text-gray-900 dark:text-white">بررسی سایر نمادها</h3>
              </div>
              
              {/* Call Candidates Grid */}
              {renderCandidateSection('سایر فرصت‌های خرید (Calls)', data.candidates.calls, 'CALL')}
              
              {/* Put Candidates Grid */}
              {renderCandidateSection('سایر فرصت‌های فروش (Puts)', data.candidates.puts, 'PUT')}
            </div>
          )}
        ...
        <div className="p-4 border-t border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-950 flex justify-end gap-3 sticky bottom-0">
          <button
            onClick={onClose}
            className="px-8 py-3 bg-gray-900 dark:bg-white text-white dark:text-black rounded-xl text-base font-bold hover:opacity-9 transition-all shadow-lg hover:shadow-xl transform active:scale-95"
          >
            بستن پنجره
          </button>
        </div>
