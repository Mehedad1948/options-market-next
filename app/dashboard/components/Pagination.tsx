    ...
    <div className="flex flex-col items-center justify-center py-6 border-t border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 rounded-b-2xl">
      ...
      <div className="hidden sm:flex  px-4 justify-between w-full sm:items-center sm:gap-2">
        <p className="text-sm text-gray-500 dark:text-gray-400 ml-4">
          صفحه <span className="font-semibold text-gray-900 dark:text-white">{currentPage}</span> از {totalPages}
        </p>

        <nav className="isolate inline-flex gap-1" dir="ltr" aria-label="Pagination">
          <button
            onClick={() => handlePageChange(currentPage - 1)}
            disabled={currentPage <= 1}
          ...
          {pageNumbers.map((page) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "inline-flex cursor-pointer items-center justify-center w-9 h-9 text-sm font-medium rounded-lg transition-all  focus:outline-none focus:ring-2 focus:ring-offset-1 dark:focus:ring-offset-gray-900",
                  isActive
                    ? "bg-blue-600 text-white shadow-md shadow-blue-500/20 focus:ring-blue-500"
                    : "text-gray-700 bg-transparent hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-800 focus:ring-gray-200 dark:focus:ring-gray-700"
                )}
