...
const getDateTimeInfo = (dateInput: string | Date) => {
  if (!dateInput) return { mainLabel: '-', subLabel: '', className: 'text-gray-400', isToday: false };
...
// --- Component ---
export function SignalRow({ signal, date, call, put }: any) {
  ...
  const handleRowClick = () => {
    if (hasSuggestion) setShowModal(true);
  };
    ...
    <>
      <tr 
        ...
        className={cn(
          ...
          isToday ? "border-r-4 border-r-emerald-500 bg-emerald-50/10" : "border-r-4 border-r-transparent",
          
          hasSuggestion ? "cursor-pointer" : "",
          // Hover effects
          isStraddle 
            ? "hover:bg-indigo-50 dark:hover:bg-indigo-900/20" 
            : hasSuggestion && "hover:bg-gray-50 dark:hover:bg-gray-800/50"
        )}
      >
        {/* Date Column */}
        <td className="px-6 py-4 whitespace-nowrap text-right">
          <div className={cn("flex items-center gap-2", "flex-col items-start gap-1")}>
            <span className={cn("text-sm", className)}>
              {mainLabel}
            </span>
            <span className={cn(
                "text-xs  dir-ltr text-right",
                isToday ? "text-emerald-600/70 dark:text-emerald-400/70 font-medium" : "text-gray-400 dark:text-gray-500"
            )}>
              {subLabel}
            </span>
          </div>
      ...
      <div className="flex flex-col justify-center">
        <div className="flex items-center gap-2">
            ...
            <span className={cn(
                ...
                isCall ? "text-emerald-700 bg-emerald-100/50 dark:bg-transparent dark:text-emerald-500" : "text-violet-700 bg-violet-100/50 dark:bg-transparent dark:text-violet-500"
            )}>
                {isCall ? 'Call' : 'Put'}
            </span>
        </div>
        ...
        <div className="flex items-baseline gap-1">
          <span className={cn(
            ...
            isCall ? "text-emerald-700 dark:text-emerald-400" : "text-violet-700 dark:text-violet-400"
          )}>
            {formatPrice(data.entry_price)}
          </span>
          <span className="text-[10px] text-gray-400 dark:text-gray-500">ریال</span>
        </div>
      </div>
    </div>
  );
