...
function LoginFormContent() {
  ...
  const [localMethod, setLocalMethod] = useState<'telegram' | 'sms'>(
    methodParam as 'telegram' | 'sms',
  );
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      // Direct function call instead of fetch('/api/...')
      const response = await verifyOtpAction(phoneParam, otp);
    ...
    <div className='w-full max-w-md bg-white dark:bg-gray-900 rounded-2xl shadow-xl p-8 border border-gray-100 dark:border-gray-800 transition-all duration-300'>
      ...
      <div className='text-center mb-8 relative'>
        {step === 'verify' && (
          <button
            onClick={goBack}
            className='absolute right-0 top-1 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition-colors'
          >
...
export default function LoginPage() {
  return (
    <div
      className='min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-950 p-4'
      dir='rtl'
    >
      <Suspense
        fallback={<Loader2 className='w-8 h-8 animate-spin text-gray-400' />}
      >
        <LoginFormContent />
      </Suspense>
    </div>
